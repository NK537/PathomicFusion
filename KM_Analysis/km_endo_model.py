"""
KM_Analysis/km_endo_model.py

Kaplan-Meier survival curves from the endoscopy model's out-of-fold predictions.
Patients are split into High / Low risk by the median predicted risk score.

Prerequisites:
    Run main_picasso.py first — it saves picasso_oof_predictions.csv
    (columns: patient_id, risk_score, surv_time, event, fold)

Run from project root:
    python KM_Analysis/km_endo_model.py
"""
...
ROOT     = _ROOT
OOF_CSV  = os.path.join(ROOT, "picasso_oof_predictions.csv")
CV_CSV   = os.path.join(ROOT, "picasso_cv_results.csv")
...
def cindex_bar_subplot(ax, cv_df):
    ...
    ax.set_ylim(0, 1.0)
    ax.set_title("C-index per Fold — Endoscopy Model", fontsize=12,
...
def main():
    print("=" * 60)
    print("KM Analysis — Endoscopy Model (Out-of-Fold Predictions)")
    print("=" * 60)

    # ── Load OOF predictions ──────────────────────────────────────────────────
    assert os.path.exists(OOF_CSV), (
        f"\nERROR: {OOF_CSV} not found.\n"
        "Run  python main_picasso.py  first to generate OOF predictions."
    )
    ...
    # Panel 3: C-index per fold bar chart
    if cv_df is not None and n_cols == 3:
        cindex_bar_subplot(axes[2], cv_df)

    plt.tight_layout()
    out_png = os.path.join(OUT_DIR, "km_endo_model.png")
    out_pdf = os.path.join(OUT_DIR, "km_endo_model.pdf")
    ...
    # Save enriched CSV
    oof_out = os.path.join(OUT_DIR, "oof_predictions_with_groups.csv")
    oof.to_csv(oof_out, index=False)
    print(f"Saved CSV: {oof_out}")

    print(f"\nLog-rank p (median split)  : {p1:.4f}")
    if not np.isnan(p2):
